...
(14 qubits, 8 iterations, $P(\text{success}) \approx 99.3\%$), and the oracle is
...
$t^*\!=\!8$, $P(\text{success})\!\approx\!99.3\%$.
\textbf{Note:} If $t^*$ is exceeded, the probability \emph{decreases} --- the
algorithm "overshoots" the solution. This phenomenon has no classical analog.
...
The two valid solutions, $[1,3,0,2]$ and $[2,0,3,1]$, concentrate $\sim\!99.3\%$
...
\begin{enumerate}
    \item \textbf{Verified Correctness.}
          For $N\!=\!4$, the full algorithm finds the 2 solutions with $P \approx 99.3\%$.
...
\small
    ...
    \bibitem{naik2018}
    R.~Naik, P.~Bhat, and M.~Samaga,
    ``A novel quantum N-queens solver algorithm and its simulation
    and application to satellite communication using IBM Quantum Experience,''
    arXiv:1806.10221 [quant-ph], 2018.

    \bibitem{nahimovs2023}
    N.~Nahimovs and A.~Locans,
    ``A quantum approach to solve N-queens problem,''
    arXiv:2312.16312 [quant-ph], 2023.

    \bibitem{tabi2020}
    Z.~Tabi, K.~H.~El-Safty, Z.~Kallus, P.~H\'aga, T.~Kozsik,
    A.~Glos, and Z.~Zimbor\'as,
    ``A quantum N-queens solver,''
    \textit{Quantum}, vol.\,4, p.\,265, 2020.
